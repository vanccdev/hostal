...
import { cancelReservationAction } from "@/app/actions/cancelaciones";
...
export const CancelReservationDialog = ({
  ...
  const [state, action, pending] = useActionState(cancelReservationAction, initialActionState);
  ...
  const canCancel = reserva.estado === "pendiente_pago" || reserva.estado === "confirmada" || reserva.estado === "checkin";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Se liberará la habitación y se guardará el monto final que conserva el hostal según la política configurada.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-[#d8d4c8] bg-[#f8f5ec] p-4 text-sm dark:border-[#314237] dark:bg-[#142019]">
            <div className="flex items-start gap-2">
              ...
              <div className="space-y-2">
                <p className="font-semibold text-[#18221b] dark:text-zinc-100">Cálculo contable</p>
                <dl className="grid gap-2 sm:grid-cols-2">
                  <Metric label="Check-in" value={formatDateTime(policy.checkinAt.toISOString())} />
                  <Metric label="Corte reembolso total" value={formatDateTime(policy.cutoffAt.toISOString())} />
          ...
          <form action={action} className="space-y-3">
            ...
            <div className="space-y-2">
              <Label htmlFor={`motivo-${reserva.id}`}>Motivo</Label>
              <Textarea
                id={`motivo-${reserva.id}`}
                name="motivo"
                required
                minLength={5}
                placeholder="Ej. Cliente solicita cancelar por cambio de planes."
              />
            </div>
            <DialogFooter>
