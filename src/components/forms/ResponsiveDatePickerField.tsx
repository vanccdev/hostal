...
type ResponsiveDatePickerFieldProps = {
  id: string;
  name: string;
  defaultValue?: string;
  disablePast?: boolean;
...
const todayValue = () => valueFromDate(startOfToday());

export const ResponsiveDatePickerField = ({
  id,
  name,
  defaultValue,
  disablePast = false,
...
}: ResponsiveDatePickerFieldProps) => {
  ...
  const [visibleMonth, setVisibleMonth] = useState<Date>(
    () => selectedDate ?? new Date(Math.min(toYear, new Date().getFullYear() - 18), 0, 1),
  );
  ...
  const updateDate = (nextValue: string) => {
    const nextDate = dateFromValue(nextValue);
    setDate(nextDate);
    if (nextDate) {
  ...
  return (
    <div className="w-full">
      <input id={`${id}-value`} name={name} type="hidden" value={fieldValue} required={required} readOnly />

      <input
        id={id}
        type="date"
        value={fieldValue}
        min={disablePast ? todayValue() : undefined}
        required={required}
        aria-label={placeholder}
        onChange={(event) => updateDate(event.target.value)}
        className="h-12 w-full rounded-md border border-input bg-background px-4 text-base text-foreground shadow-sm outline-none ring-offset-background transition-colors focus-visible:ring-2 focus-visible:ring-ring lg:hidden"
      />
        ...
        <Popover>
          <PopoverTrigger asChild>
            ...
            >
              ...
              {labelFromDate(selectedDate) ?? (
                <span className="text-[#66736a] dark:text-[#b7c0b4]">{placeholder}</span>
              )}
            </Button>
          </PopoverTrigger>
          <PopoverContent align="start" collisionPadding={16} className="w-[min(22rem,calc(100vw-2rem))] overflow-hidden">
            ...
            <Calendar
              ...
              onSelect={(nextDate) => updateDate(valueFromDate(nextDate))}
            />
          </PopoverContent>
        </Popover>
      </div>
