...
import type { Cancelacion } from "@/types/database";
  ...
  if (query && tableQuery.q) {
    query =
      tableQuery.qColumn === allColumnsValue
        ...
        : query.ilike(tableQuery.qColumn as "motivo", ilikePattern(tableQuery.q));
  }

  const { data, count } = query ? await query : { data: [], count: 0 };
  const cancelaciones = data ?? [];
  const userNamesById = await userNamesByIdForRows(supabase, cancelaciones);

  return (
    <section className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Cancelaciones</h1>
        <p className="text-sm text-zinc-600 dark:text-zinc-400">Solicitudes y registros de cancelación propios.</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Mis cancelaciones</CardTitle>
        </CardHeader>
        <CardContent>
          <DataTable<Cancelacion>
            data={cancelaciones}
            empty="No hay cancelaciones registradas."
            columns={columnsForTable<Cancelacion>("cancelaciones", cancelaciones, { userNamesById })}
