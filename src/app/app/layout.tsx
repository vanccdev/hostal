...
export default async function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  const shouldCompleteProfile = !currentUser.profile?.must_change_password && isClientProfileIncomplete(guest, currentUser.phone);
  ...
  return (
    <div className="min-h-screen bg-[#f6f1e6] dark:bg-[#101a14]">
      ...
      {shouldCompleteProfile ? (
        <CompleteClientProfileDialog guest={guest} initialPhone={currentUser.phone} profileName={currentUser.profile?.nombre ?? ""} />
      ) : null}
    </div>
  );
}
